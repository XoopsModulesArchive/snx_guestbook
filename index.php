...
function showSignForm() {
	...
	<tr><td align=right>Homepage:</td><td><input name="url" size=30 value="http://"></td></tr>
	<tr><td align=right>'._SnX_GB_TITLE.'<font color=#ff0000>*</font>:</td><td><input name="title" size=30></td></tr>
	<tr><td align=right>Message<font color=#ff0000>*</font> (no HTML):</td><td><textarea name="comment" rows=5 cols=50></textarea></td></tr>
...
function view($from) {
	...
	list($id, $epp, $notifyEmail, $moderate, $dateFormat) = $xoopsDB->fetchRow($result);
	$epp = ($epp?$epp:5);
	
	$nbEntries=$xoopsDB->getRowsNum($xoopsDB->query("select id from ".$xoopsDB->prefix("snx_guestbook").";"));
	
	$sql="select id,UNIX_TIMESTAMP(date),country,city,name,title,comment,homepage,email,icon,ip,accepted from ".$xoopsDB->prefix("snx_guestbook")." order by date desc LIMIT $from,$epp;";
	$result=$xoopsDB->query($sql);
	...
	$count=0;
	while(list($id,$date,$country,$city,$name,$title,$comment,$homepage,$email,$icon,$ip,$accepted)=$xoopsDB->fetchRow($result)) {
		if(($moderate && $accepted) || !$moderate) {
			...
			echo "\t<p class=\"GB_date\"><span>".date($dateFormat, $date)."</span></p>\n";
			if($email!="") {
	...
	if($nbEntries>$epp) {
		...
		$pagenav=new XoopsPageNav($nbEntries, $epp, $from, 'from');
...
//	echo $myPost["fullname"];
	$sql="insert into ".$xoopsDB->prefix("snx_guestbook")." values('','".$myPost["site"]."','".$myPost["icon"]."','".$myPost["fullname"]."','".$myPost["email"]."','".$myPost["url"]."','".$myPost["title"]."','".strip_tags($myPost["comment"])."',NOW(),'".$HTTP_SERVER_VARS["REMOTE_ADDR"]."','".$myPost["country"]."','".$myPost["city"]."', '".($moderate?"0":"1")."');";
	$result=$xoopsDB->query($sql);
	if(!$result) {
		echo "MySQL insert error";
	} else {
		if(!empty($notifyEmail)) {
			$subject = $msg = "\"{$myPost['fullname']}\" <{$myPost['email']}> "._SnX_MSG_POSTED."\n\n";
			if($moderate) {
				$msg .= "Message: \n".strip_tags($myPost["comment"]);
			}
			mail($notifyEmail, $subject, $msg, "From: ".($myPost['email']?$myPost['email']:$notifyEmail));
		}
		if($moderate) {
			header("Location: ".XOOPS_URL."/modules/snx_guestbook/?op=moderated");
		} else {
...
function moderated() {
	echo "<center><h3>"._SnX_MSG_MODERATED."</h3>";
	echo "<br><br><a href=\"".XOOPS_URL."/modules/snx_guestbook/?op=view\">"._SnX_CONTINUE."</a>";
	echo "</center>";
}
